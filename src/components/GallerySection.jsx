...
export default function GallerySection() {
  ...
  return (
    <section className="relative py-24 md:py-32 skew-divider overflow-hidden flex flex-col">
      <div className="max-w-7xl mx-auto px-6 md:px-10 w-full mb-14">
        <motion.h2
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.7, ease: 'easeOut' }}
          className="font-display text-4xl md:text-6xl lg:text-7xl uppercase tracking-[0.06em] leading-[1.05] text-[#F1F5F9]"
        >
          The Energy Was Real
        </motion.h2>
      </div>
        ...
        <div className="w-full">
          <div className="animate-marquee gap-4">
            {seamlessRow1.map((item, i) => (
              <div
                key={`r1-${i}`}
                className="group relative w-72 md:w-[28rem] h-48 md:h-72 bg-[#0F0F1A] rounded-sm overflow-hidden cursor-pointer shrink-0"
